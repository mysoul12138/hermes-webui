...
def test_projected_sidebar_rows_do_not_run_legacy_lineage_collapse():
    js = SESSIONS_JS_PATH.read_text(encoding="utf-8")
    source = f"""
const src = {js!r};
function extractFunc(name) {{
...
eval(extractFunc('_isChildSession'));
eval(extractFunc('_isProjectedSessionRow'));
eval(extractFunc('_projectedSidebarRow'));
eval(extractFunc('_sidebarRowsFromProjectionOrLegacy'));
let collapseCalls = 0;
function _collapseSessionLineageForSidebar() {{
  collapseCalls += 1;
  throw new Error('legacy collapse should not run for projected rows');
}}
function _attachChildSessionsToSidebarRows() {{
  throw new Error('legacy child attach should not run for projected rows');
}}
const rows = _sidebarRowsFromProjectionOrLegacy([
  {{
    session_id:'root',
    projection_version:1,
    canonical_session_id:'root',
    represented_session_ids:['root'],
    lineage_segments:[{{session_id:'root'}}],
    child_sessions:[],
  }},
  {{
    session_id:'tip',
    ...
    lineage_segments:[{{session_id:'tip'}}],
    child_sessions:[],
  }},
]);
console.log(JSON.stringify({{collapseCalls, ids: rows.map(row => row.session_id)}}));
"""
    assert json.loads(_run_node(source)) == {"collapseCalls": 0, "ids": ["root", "tip"]}


def test_projected_sidebar_rows_keep_backend_child_sessions_without_duplicate_attach():
    js = SESSIONS_JS_PATH.read_text(encoding="utf-8")
    source = f"""
const src = {js!r};
function extractFunc(name) {{
...
const rows = _sidebarRowsFromProjectionOrLegacy([
  {{
    session_id:'parent',
    projection_version:1,
    canonical_session_id:'parent',
    represented_session_ids:['parent'],
    lineage_segments:[{{session_id:'parent'}}],
    child_sessions:[{{session_id:'child', title:'Child', relationship_type:'child_session', parent_session_id:'parent'}}],
  }},
  {{
    session_id:'child',
    projection_version:1,
    canonical_session_id:'child',
    represented_session_ids:['child'],
    lineage_segments:[{{session_id:'child'}}],
    child_sessions:[],
    relationship_type:'child_session',
...
"""
    ...
    assert rows[0]["_child_session_count"] == 1
    assert [child["session_id"] for child in rows[0]["_child_sessions"]] == ["child"]


def test_projected_child_only_row_remains_visible_as_orphan_when_parent_filtered_out():
    js = SESSIONS_JS_PATH.read_text(encoding="utf-8")
    source = f"""
const src = {js!r};
function extractFunc(name) {{
...
"""
    ...
    assert rows[0]["_orphan_child_session"] is True
    assert rows[0]["_lineage_key"] == "child"


def test_projected_search_child_without_parent_remains_visible_as_orphan():
    js = SESSIONS_JS_PATH.read_text(encoding="utf-8")
    source = f"""
const src = {js!r};
function extractFunc(name) {{
...
const rows = _sidebarRowsFromProjectionOrLegacy([
  {{
    session_id:'matched-child',
    title:'Search hit in child task',
    projection_version:1,
    canonical_session_id:'matched-child',
    represented_session_ids:['matched-child'],
    lineage_segments:[{{session_id:'matched-child'}}],
    child_sessions:[],
    relationship_type:'child_session',
    parent_session_id:'filtered-parent',
    search_match:true,
  }},
]);
console.log(JSON.stringify(rows));
"""
    rows = json.loads(_run_node(source))
    assert [row["session_id"] for row in rows] == ["matched-child"]
    assert rows[0]["_orphan_child_session"] is True
    assert rows[0]["search_match"] is True


def test_projected_active_child_without_parent_matches_sidebar_active_state():
    js = SESSIONS_JS_PATH.read_text(encoding="utf-8")
    source = f"""
const src = {js!r};
function extractFunc(name) {{
...
const rows = _sidebarRowsFromProjectionOrLegacy([
  {{
    ...
    projection_version:1,
    canonical_session_id:'active-child',
    represented_session_ids:['active-child'],
    lineage_segments:[{{session_id:'active-child'}}],
    child_sessions:[],
    relationship_type:'child_session',
...
console.log(JSON.stringify({{
  ids: rows.map(row => row.session_id),
  orphan: rows[0]._orphan_child_session,
  active: _sessionLineageContainsSession(rows[0], 'active-child'),
}}));
"""
...
def test_legacy_sidebar_rows_still_use_lineage_collapse_fallback():
    js = SESSIONS_JS_PATH.read_text(encoding="utf-8")
    source = f"""
const src = {js!r};
function extractFunc(name) {{
...
eval(extractFunc('_sidebarRowsFromProjectionOrLegacy'));
const rows = _sidebarRowsFromProjectionOrLegacy([
  {{session_id:'root', title:'Root', updated_at:10, last_message_at:10, _lineage_root_id:'root'}},
  {{session_id:'tip', title:'Tip', updated_at:20, last_message_at:20, _lineage_root_id:'root'}},
]);
console.log(JSON.stringify(rows));
"""
    rows = json.loads(_run_node(source))
    assert [row["session_id"] for row in rows] == ["tip"]
    assert rows[0]["_lineage_collapsed_count"] == 2
    assert [seg["session_id"] for seg in rows[0]["_lineage_segments"]] == ["tip", "root"]
